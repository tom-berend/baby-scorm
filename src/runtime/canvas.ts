...
export class Canvas {
    ...
    constructor(canvasTag: string) {
        console.log('in Canvas construtor')
    ...
    canvasKeypress(event: KeyboardEvent) {
        ...
        this.mouseObservable.notifyObservers('keypress', event)
    }
...
class DrillTools extends Canvas {
    ...
    public maxTime: number  // seconds
    public overTime: boolean // set to true if went overtime
    public errorKey: boolean  // set to true if ANY wrong keys
    public inputBoxPosition: V2
...
export function drillByName(name: string, canvasTag: string, n: number) {
    ...
    console.assert(drills[name] !== undefined, `could not find a method with the name '${name}'`)
...
export class Drills extends DrillTools {
    ...
    public drawQuestion: (q: IQuestion) => void
